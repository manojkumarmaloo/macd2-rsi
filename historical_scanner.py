...
def backtest_symbol(df: pd.DataFrame, symbol: str, tf: str, p: Params,
                    forward_bars: list[int]) -> pd.DataFrame:
    """
    Vectorized version of evaluate() that scans EVERY historical bar instead of
    just the last one, and attaches forward returns so you can judge whether the
    signal actually led anywhere. One row per historical occurrence.
    """
    ...
    n = len(df)
    min_bars = max(p.slow2 + p.signal2, p.rsi_len, p.lbL + p.lbR) + 10
    if n < min_bars:
        return pd.DataFrame()

    o = rsi(close, p.rsi_len).to_numpy()
    macd2 = ema(close, p.fast2) - ema(close, p.slow2)
    hist2 = (macd2 - ema(macd2, p.signal2)).to_numpy()

    rsi_ok = o > 50.0
    macd_blue = np.zeros(n, dtype=bool)
    macd_blue[2:] = (hist2[2:] >= 0.0) & (hist2[:-2] < hist2[2:])
    signal_mask = rsi_ok & macd_blue
    signal_mask[:min_bars] = False                  # don't count the warm-up window

    div_sig = bullish_divergence(o, low, p.lbL, p.lbR, p.range_lower, p.range_upper)
    div_idx = np.flatnonzero(div_sig)

    rows = []
    closes = close.to_numpy()
    for i in np.flatnonzero(signal_mask):
        prior = div_idx[div_idx <= i]
        div_bars_ago = int(i - prior[-1]) if len(prior) else None
        if p.require_div and (div_bars_ago is None or div_bars_ago > p.div_lookback):
            continue
        row = {
            "symbol": symbol, "timeframe": tf, "date": df.loc[i, date_col],
            "close": closes[i], "rsi": o[i], "hist2": hist2[i],
            "div_bars_ago": div_bars_ago,
        }
        for fb in forward_bars:
            j = i + fb
            row[f"fwd_ret_{fb}b_%"] = (closes[j] / closes[i] - 1) * 100 if j < n else np.nan
        rows.append(row)
    return pd.DataFrame(rows)


def run_backtest(symbols: list[str], timeframes: list[str], p: Params,
                 forward_bars: list[int], batch_size: int) -> pd.DataFrame:
    all_rows = []
    for tf in timeframes:
        for i in range(0, len(symbols), batch_size):
            ...
            for sym, df in frames.items():
                try:
                    res = backtest_symbol(df, sym, tf, p, forward_bars)
    ...
    print(f"\n{len(out)} historical signals across {len(symbols)} symbols "
         f"/ {timeframes}", file=sys.stderr)
    for col in [c for c in out.columns if c.startswith("fwd_ret_")]:
        vals = out[col].dropna()
        if len(vals):
            print(f"  {col}: mean {vals.mean():+.2f}%  win-rate "
                 f"{(vals > 0).mean() * 100:.0f}%  n={len(vals)}", file=sys.stderr)
    return out
...
PERIOD_OVERRIDE: str | None = None   # set from --period at startup
...
def fetch_batch(symbols: list[str], tf: str) -> dict[str, pd.DataFrame]:
    ...
    period = PERIOD_OVERRIDE or PERIOD_FOR.get(tf, "1y")
...
def main() -> None:
    ...
    ap.add_argument("--backtest", action="store_true",
                    help="scan full history instead of just the latest bar")
    ap.add_argument("--forward-bars", nargs="+", type=int, default=[5, 10, 20],
                    help="bars ahead to measure return for, e.g. 5 10 20")
    ap.add_argument("--out", default="backtest_results.csv",
                    help="CSV path for --backtest output")
    ap.add_argument("--period", default=None,
                    help="override yfinance lookback, e.g. 60d, 2y, 5y "
                        "(15m data is capped near 60d by Yahoo regardless)")
    args = ap.parse_args()

    global PERIOD_OVERRIDE
    ...
    if args.backtest:
        results = run_backtest(symbols, args.timeframes, p, args.forward_bars,
                               args.batch_size)
        if results.empty:
            print("No historical signals found.", file=sys.stderr)
        else:
